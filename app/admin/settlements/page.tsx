...
import { Search, Wallet, CheckCircle, TrendingUp, X } from "lucide-react"
...
function SettleDrawer({s,onClose}:{s:typeof ALL[0]|null;onClose:()=>void}){
    if(!s)return null
...
export default function SettlementsPage() {
    const [filter,setFilter]=useState("all"); const [q,setQ]=useState(""); const [open,setOpen]=useState<typeof ALL[0]|null>(null)
    ...
    return(<><h1 className="adm-page-title">تسویه‌حساب‌ها</h1>
        <div className="adm-stat-grid">
            ...
            <div className="adm-stat"><div className="adm-stat__top"><div className="adm-stat__label"><CheckCircle size={18}/>کل پرداختی</div></div><div className="adm-stat__value">{faNum(totalPaid)}<span className="adm-stat__unit">تومان</span></div><div className="adm-stat__compare">میانگین: {faNum(Math.round(totalPaid/paid.length))} تومان</div></div>
